...
/// Maximum number of messages before triggering context compression.
/// System prompt (index 0) is always preserved.
const CONTEXT_WINDOW_MAX: usize = 40;

/// After compression, keep this many recent messages (plus system prompt).
const CONTEXT_WINDOW_KEEP: usize = 10;
...
pub struct Agent {
    ...
    /// Total messages processed (for stats, doesn't reset on compression)
...
impl Agent {
    ...
    /// Compress the conversation context when it exceeds `CONTEXT_WINDOW_MAX`.
    ///
    /// Keeps: system prompt (index 0) + summary of old messages + recent N messages.
    /// The summary is generated by the fast model to save time/cost.
    async fn maybe_compress_context(&mut self) {
        if self.messages.len() <= CONTEXT_WINDOW_MAX {
            return;
        }

        tracing::info!(
            "Context compression triggered: {} messages → keeping {}",
            self.messages.len(), CONTEXT_WINDOW_KEEP
        );

        // Split: system prompt | old messages to summarize | recent messages to keep
        let system_prompt = self.messages[0].clone();
        let keep_start = self.messages.len() - CONTEXT_WINDOW_KEEP;
        let old_messages: Vec<Message> = self.messages[1..keep_start].to_vec();
        let recent_messages: Vec<Message> = self.messages[keep_start..].to_vec();
        ...
        let summary = match self.fast_llm.chat(vec![
            Message { role: "system".to_string(), content: "Summarize concisely.".to_string() },
            Message { role: "user".to_string(), content: summary_prompt },
        ]).await {
            Ok(resp) => resp.content,
            Err(_) => {
                // Fallback: just truncate without summary
                let items: Vec<String> = old_messages.iter()
                    .take(5)
                    .map(|m| format!("- {}: {}", m.role, m.content.chars().take(80).collect::<String>()))
                    .collect();
                format!("(compressed {} messages)\n{}", old_count, items.join("\n"))
            }
        };
        ...
        tracing::info!(
            "Context compressed to {} messages (was {})",
            self.messages.len(), CONTEXT_WINDOW_MAX + 1
        );
    }
